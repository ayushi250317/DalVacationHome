...
const AdminMessaging = () => {
  ...
  useEffect(() => {
    const fetchQueries = async () => {
      if (email) {
        try {
          const response = await axios.get(`https://us-central1-csci-5410-428021.cloudfunctions.net/getAgentQueries?username=${email}`);
  ...
  const handleChatClick = (bookingRefCode) => {
    ...
    navigate(`/adminchat/${bookingRefCode}`);
  };

  return (
    <Container>
      <Typography variant="h4" gutterBottom>Issued Tickets</Typography>
